...
CHECKPOINT_SAVE_DIR="/model/taishi/checkpoint/long-context/Swallow-13b-hf/lr_${LR}-minlr_${MIN_LR}_warmup_${LR_WARMUP_STEPS}_seq_${SEQ_LENGTH}"
...
# job name
JOB_NAME="Swallow-13b-hf-${NODE_TYPE}-${NUM_NODES}node-${NUM_GPUS}gpu-${SEQ_LENGTH}s-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}-WARMUP=${LR_WARMUP_STEPS}-WD=${WEIGHT_DECAY}-GC=${GRAD_CLIP}"
...
mpirun -np $NUM_GPUS \
  ...
  --wandb-name "${JOB_NAME}" \
  --base-model tokyotech-llm/Swallow-13b-hf \
